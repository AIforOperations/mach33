...
- `klaviyo.py` — `upload | create | patch | render | get | list | checkenv | stores | fonts | wire | whichstore | learnfigma` (reads the key from env or the repo-root `.env`; **`--store <slug>`** selects a wired account — see Multi-store). `fonts` lists the account's hosted fonts; `fonts --has "<family>"` tests one (PRESENT/ABSENT) for the Figma-font-first check.
